...
select substrb(hiredate, 0, 5) as year_date from emp;
...
select trunc(sysdate - to_date('20200101', 'YYYYMMDD')) as days_passed from dual;

-- 21. 사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 NULL 값 대신 0으로 출력하시오.
select ename, nvl(mgr, 0) as supervisor from emp;

-- 22. DECODE 함수로 직급에 따라 급여를 인상하도록 하시오. 직급이 ‘ANALIST”인 사원은 200, ‘SALESMAN’인 사원은 180, ‘MANAGER’인 사원은 150, ‘CLERK”인 사원은 100을 인상하시오.
select ename, job, sal, decode(job, 'ANALYST', sal+200, 'SALESMAN', sal+180, 'MANAGER', sal+150, 'CLERK', sal+100) as sal_raised from emp; 

-- 23. 모든 사원의 급여 최고액, 최저액, 총액 및 평균 급여를 출력하시오. 평균에 대해서는 정수로 반올림하시오.
select max(sal) as max_sal, min(sal) as min_sal, sum(sal) as total_sal, round(avg(sal)) as avg_sal from emp;

-- 24. 각 담당 업무 유형별로 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. 평균에 대해서는 정수로 반올림 하시오.
select job, max(sal) as max_sal, min(sal) as min_sal, sum(sal) as total_sal, round(avg(sal)) as avg_sal from emp group by job;
...
select deptno, count(*) as numofemp, round(avg(sal), 2) as avg_sal from emp group by deptno;
